...
% Fonction qui permet d ajouter un element en fin de liste
ajoutFin(X,[],[X]).
...
% Fonction qui renvoie le nième element d une liste 
/* Paramètres : N index de l element qu on veut recuperer, L liste, X element retourne */
...
% Definition et test des conditions de fin de jeu
/* Paramètres : G grille, J joueur */
...
/* Paramètres : J joueur */
gagnant(J):-write('Le Joueur '), write(J), write(' a gagne !').
...
%Si un coup permet a l adversaire de gagner on se defend(coup defensif).
jouerIA(G):-   coupGagnant(C,G,x), enregistrerCoupIA(C,G,o,X,G),
			   afficherGrille(X),
			   write('\n'),
			   jouerCoupJoueur(X).

jouerIA(0, G):- write('Pas de coup trouve').
...
%Si on a pas de coup immediat on fait un coup au centre ou au plus près possible pour une victoire possible en verticale.
%E correspond au nombre de case restante dans la colonne (on cherche à jouer dans une colonne où l on peut gagner.
